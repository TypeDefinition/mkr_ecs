...
namespace mkr {
    ...
    struct archetype {
        ...
        static std::unordered_map<uint64_t, std::function<void(archetype *, uint64_t)>> remove_funcs_;
        static std::unordered_map<uint64_t, std::function<void(archetype *, archetype *, uint64_t)>> copy_funcs;
        ...
        static void register_component_type() {
            ...
            remove_funcs_[family_id] = [=](archetype *_arc, uint64_t _index) {
                auto *arr = static_cast<std::vector<C> *>(_arc->components_[family_id]);
                arr->erase(arr->begin() + _index);
            };
            copy_funcs[family_id] = [=](archetype *_src, archetype *_dst, uint64_t _src_index) {
        ...
        template<class C>
        static uint64_t add_component(archetype *_arc, C _component) {
            const uint64_t family_id = FAMILY_ID(component, C);
            auto *arr = static_cast<std::vector<C> *>(_arc->components_[family_id]);
        ...
        static void copy_components(archetype *_src, archetype *_dst, uint64_t _index, const component_bitset &_bitset) {
            for (uint64_t i = 0; i < component::MAX_TYPES; ++i) {
                if (_bitset[i]) {
        ...
        static void remove_components(archetype *_arc, uint64_t _index) {
        ...
        template<class C>
        static const C* get_component(archetype *_arc, uint64_t _index) {
            const uint64_t family_id = FAMILY_ID(component, C);
            auto *arr = static_cast<std::vector<C> *>(_arc->components_[family_id]);
